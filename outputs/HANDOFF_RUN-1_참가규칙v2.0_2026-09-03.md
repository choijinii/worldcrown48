...
>
##### B안 이행 조건 4건 (2026-09-05 대표 지시) — 전부 필수

**조건 1 — 문서 이름을 만드는 곳은 순수 함수 하나뿐이다.**
`runDocId(uid, tournamentId, runIndex)` 하나만 두고, **코드 어디서도 문자열을 직접 조합하지 않는다.**
`bracketSeedDocId` · roundProgress id · `crownCardId` · localStorage 캐시 키 · Crown Card 이미지 경로가 **전부 이 함수를 통과**한다.
> **"1회차만 예외"가 if 분기로 흩어지면 그게 다음 버그다. 이게 B안의 유일한 약점이다.** (대표)
>
> 단위 테스트로 못 박는다 — **1회차 → 접미사 없음 · 2회차 → `_r2` · 5회차 → `_r5`**. 경계 3건 모두 필수.
>
> **배치**: `lib/arena/runDocId.ts` 에 두고 기존 관례대로 `functions/scripts/copy-run.mjs` 로 functions에 미러링한다
> (리포에 이미 `copy-crown` · `copy-ranking` · `copy-news` · `copy-embed` 4개가 있다).
> 같은 방식으로 **`decideRun` 도 공용 모듈**로 둔다 — 클라이언트·서버가 문자 그대로 같은 코드를 실행하는 것이
> §9 함정 5(두 게이트가 어긋나면 P0)를 구조로 막는 유일한 방법이고, 그 수단이 이미 리포에 있다.

**조건 2 — §7 수동 테스트에 "중간 구간 검증"을 넣는다.**
PR 1을 머지·배포한 **직후, PR 2를 올리기 전에** 프리뷰에서 1판 완주가 정상인지 먼저 확인한다.
**이 구간을 건너뛰고 PR 2로 넘어가지 않는다** — B안이 실제로 통하는지 확인하는 유일한 지점이다. → §7 0단계

**조건 3 — AC 11(옛 문서 = 1회차)은 지우지 않는다. 검증 방법만 갱신한다.**
폴백 코드로 만족되는 게 아니라 **"옛 문서 id가 곧 1회차 id"라는 구조로 만족**된다.
> **나중에 왜 폴백이 없는지 아무도 모르면 안 된다.** (대표) → 그래서 이 근거를 §3.0과 §4 AC 11 양쪽에 남긴다.

**조건 4 — 옛 `votes` 에 `runIndex` 가 없는 문제: 판단 기준을 먼저 정하고 실측한다.**
옛 `votes` 문서에는 `runIndex` 항목이 없어 `where("runIndex","==",1)` 에 안 걸린다
→ 전환 배포 순간 **판을 돌던 팬은 그 판의 진행이 초기화**된다(완주 기록·Crown Card는 무사).

> **배포 판단 기준 (실측 전에 확정 — 2026-09-05 대표 지시)**
> 진행 중인 판(= `roundProgress` 가 있고 `complete !== true`) 개수를 세어:
> - **0건 → 그대로 배포한다.**
> - **1건 이상 → 대표님께 보고하고 결정을 받는다. 임의 배포 금지.**
>
> ❌ 실측하고 나서 판단하지 말 것. 기준을 먼저 정해두고 실측한다.
> 일괄 변환 스크립트는 §5 DON'T 3으로 금지되어 있어 다른 길이 없다.
...
| 3 | **게스트는 이 문서로 못 센다.** `tournament_runs`는 Tournament별 문서인데 게스트 한도는 "하루 통틀어 1판"이라 Tournament를 가로지른다 | 게스트 전용 보관소 **`guest_runs/{uid}`** 를 신설한다 (§3 Phase 1 표에 반영). `{ lastRunDate, runsToday, tournamentId }`. 게스트 uid는 브라우저마다 새로 생기므로 uid 단위 문서 1개로 충분하다. KST 리셋 판정은 `tournament_runs`와 **같은 방식**을 쓴다(리셋 로직이 하나여야 테스트도 하나다). **⚠️ 회차 번호는 게스트도 `tournament_runs` 에서 받는다** — 같은 브라우저의 익명 계정은 유지되므로 내일 같은 Tournament를 또 돌면 그건 2회차다. `guest_runs` 는 **한도만** 담당한다 (설계서 §1.4) |
...
4. **미완주 판도 회차를 소모한다** — 판을 시작한 시점에 카운트한다(완주 시점 아님). 이유: 24강까지만 반복해 카드 없이 판만 태우는 구멍 차단.
   ...
   > 카운트 시점은 **아레나 진입이 아니라 그 판의 첫 선택**이다. **DO 4 위반이 아니라 DO 4의 구현 정의다.**
   >
   > **왜 구멍이 안 생기나.** 미완주 판은 `decideRun` 에서 **`continue` 로만 재진입**된다(§3.0 조건 5) —
   > 끝내지 않으면 다음 판이 아예 열리지 않는다. 그래서 "24강까지만 반복해 판만 태우는" 경로가
   > **구조적으로 존재하지 않는다.** DO 4가 막으려던 구멍은 이어하기 규칙이 이미 막고 있다.
   >
   > **왜 진입이 아니라 첫 선택인가.** 들어왔다가 한 번도 안 고르고 나간 팬에게 하루 5판 중 1판을 태우는 건
   > 규칙이 아니라 사고다. 잘못 눌러 들어온 것만으로 한도가 깎인다.
   >
   > ⚠️ 나중에 이 줄을 "DO 4 위반"으로 읽지 말 것.
...
**0단계 — 중간 구간 검증 (PR 1 머지·배포 직후, PR 2를 올리기 전. §3.0 B안 조건 2 — 건너뛰지 말 것)**

> 🔴 **대회 선택 규칙 (2026-09-06 추가 — 실제 사고로 배웠다)**
> **반드시 마감(Tournament Deadline)이 아직 남아 있는 Tournament를 고를 것.**
> 2026-09-06 P0 때 대표님이 고른 대회가 마감 3일 지난 것이었고, 그래서 "투표 실패"가
> 마감 강제인지 회차 코드의 결함인지 구분이 안 됐다. 실측 결과 `active` Tournament
> 19개 중 **14개가 이미 마감을 지나 있었다** — 아무 대회나 고르면 마감된 것을 고를
> 확률이 더 높다. 고르기 전에 `tournamentDeadline > 지금` 을 먼저 확인한다.

프리뷰에서 **옛 화면 + 새 서버** 조합으로 1판을 완주해 본다. 확인 항목:
① 라운드 전환 안내가 정상적으로 뜨는가(HF-1.6 유형 P0가 재발하지 않는가) ② THE FINAL이 멈추지 않는가 ③ Crown Card가 생성되는가.
셋 중 하나라도 실패하면 **PR 2로 넘어가지 말고 보고**한다 — B안이 안 통한다는 뜻이다.

---

이하 5단계는 **PR 2 머지 후** Vercel Preview에서 **로그인 계정**으로:
1. A Tournament 1판 완주 → 카드 1장 확인 → **[다시 도전 (2/5)]** 클릭
...
> **⛔ 배포 전제 (2026-09-06 대표 확정)**
> **§7 0단계를 수행할 수 없으면 배포를 진행하지 않는다.**
> 2026-09-06에 브라우저 확장이 연결되지 않아 0단계를 못 돌린 채 배포했고, 그 사이
> 프로덕션 투표가 막혔다. 0단계는 "배포 후에 하면 좋은 것"이 아니라 **배포의 조건**이다.
...
☐ **PR 1 안에서** `votes` 의 `runIndex` 조회 3곳의 복합 인덱스를 처리
   → **2026-09-06 완료**. 아래 §13 참조 (에뮬레이터로는 확증 불가 · 보험으로 3개 추가)
...
## §13. 복합 인덱스 — 2026-09-06 실측과 대표 결정 (지우지 말 것)

> **`firestore.indexes.json` 의 `votes` 인덱스 3개는 "안 쓰는 색인"이 아니다. 의도적으로 넣은 보험이다.**
> JSON에는 주석을 달 수 없어 근거를 여기 남긴다. 지우기 전에 반드시 이 절을 읽을 것.

### 해당 인덱스

```
votes: userId, matchId, runIndex               ← onVote 중복 방지
votes: userId, tournamentId, round, runIndex   ← advanceRound 라운드 완료 판정
votes: userId, tournamentId, runIndex          ← voteStore 진행 복원
```

### 왜 "필요한지 확인"이 아니라 "보험"인가

**Firestore 에뮬레이터는 인덱스 부족을 강제하지 않는다 — 로컬 확증이 원리적으로 불가능하다.**
대조 실험으로 확인했다: 프로덕션이라면 반드시 실패할 쿼리(`votes` 에서 `userId==` + `createdAt` 정렬,
해당 복합 인덱스 미배포)를 에뮬레이터에 던졌더니 **그냥 통과**했다.
따라서 "에뮬레이터에서 인덱스 오류가 안 났다"는 판정은 **false green**이다.

프로덕션 설정을 직접 조회한 근거로는 **불필요**해 보인다 —
① 프로덕션에 `votes(userId, tournamentId, round)` 복합 인덱스가 **없는데** `advanceRound` 가 바로 그
3중 등가 쿼리를 돌리고 있다(등가 전용 쿼리는 단일 필드 인덱스 병합으로 처리된다는 실측 증거)
② `fieldOverrides` 가 비어 있어 새 `runIndex` 도 자동으로 단일 필드 인덱스를 갖는다
③ 추가되는 `runIndex ==` 는 등가 조건이라 ①과 같은 부류다.

### 대표 결정 (2026-09-06)

> **"근거 3건이 강한 건 알지만 추론이지 확인이 아니다. 터지는 시점이 내 눈검사 중이라 왕복 비용이
> 색인 3개 값보다 크다. 나중에 규모가 커져 부담되면 그때 빼면 되지만, 막힌 투표는 되돌릴 수 없다."**

→ 인덱스 3개를 명시적으로 넣어 **질문 자체를 없앤다.** 비용은 `votes` 쓰기당 색인 3개이며 런칭 전
규모에서는 무시할 만하다. 규모가 커져 쓰기 비용이 부담되면 **그때 빼는 것이 안전한 방향**이다
(빼서 틀리면 투표가 막히지만, 넣어 두고 안 쓰면 비용만 조금 더 든다).

⚠️ 배포 시 `firebase deploy --only firestore:indexes` 가 한 번 필요하다.

---

## §14. 2026-09-06 P0 — 마감 강제와 그 설명이 갈라졌다

**증상**: 프로덕션에서 게스트가 48강 첫 매치를 고르면 "투표에 실패했어요. 다시 시도해주세요."

**원인**: PR 1이 `onVote` 에 **마감 강제**를 넣었는데, 그걸 설명하는 **화면·문구는 PR 2**에 있었다.
실제 호출로 확정한 오류: `functions/failed-precondition · details {"code":"deadline_passed"}`.
배포 시점에 `active` Tournament 19개 중 **14개가 마감을 지나** 있어(실제 팬용 4개 포함) 사실상 투표가 막혔다.

색인·권한은 원인이 아니었다 — 신규 색인 3개는 전부 `[READY]` 였고, 서버는 admin SDK라 보안 규칙을 우회한다.

**대응**: `onVote` 의 마감 강제만 껐다(`deadlinePassed: false`). `decideRun` 의 판정 로직과 테스트는 그대로다.

> **📌 규칙 — 막는 것과 왜 막혔는지 알려주는 것은 한 쌍이다 (2026-09-06 대표 확정)**
> 그 짝이 갈라진 것이 이번 사고다. **강제는 반드시 그 문구·화면과 같은 PR에서 켠다.**
> PR 2가 §8 `arena.run.deadlinePassed` 와 첫 진입 안내(AC 16)를 올릴 때 함께 되살린다.
> ⚠️ 화면 처리 없이 `deadlinePassed` 만 `true` 로 되돌리지 말 것 — 같은 P0가 재발한다.

**배운 것**: PR 1 본문에 "이 PR은 프로덕션 화면 동작을 바꾸지 않는다"고 적었으나 **거짓이었다.**
서버가 예전에 받던 투표를 거부하기 시작했다. "화면 코드를 안 바꿨다"와 "화면 동작이 안 바뀐다"는 다르다.

---

## §15. 런칭 전 정리 대상 (지금 손대지 말 것 — 2026-09-06 대표 지시)

**마감이 몇 주 지난 Tournament 14개가 아직 `status: active` 로 화면에 살아 있다.**
그중 4개는 시드가 아니라 실제 팬용이다(kpop 여자 아이돌 / Kpop 아티스트 / 남자 아이돌 댄스 / 틱톡커).
나머지는 `a1-preview-*` · `admin-preview-*` · `테스트 토너먼트` 계열이다.
