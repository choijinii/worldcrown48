...
### §12-E · PR 1 종료 기록 (2026-09-20)
- PR #103 머지(main 7bb4a93) · rules·functions 배포 · fix-loop-15 48건 · 스모크 3화면 합격 · **대표 눈검사 합격(2026-09-20 16:50 KST)**.
- 눈검사에서 나온 지적 배정: 메뉴바·☰ → NAV-1 / 이름 다국어 → NAME-I18N-1 / 결승 3분할·게스트 안내 문구 제거·가로 전체화면 → PR 2 / 쿠키 동의 바 → COOKIE-1(PR #104 초안).
- 보류: 세로 숏츠 정사각(운영에 세로 참가자 없음) · Lighthouse 73(FONT-1).

### §12-F · PR 2 분할 기록 (2026-09-20)
- **PR 2 = 2a + 2b.**
  - **2a**(이 PR): 모바일 가로 첫 탭 전체화면 · 매치 화면 게스트 안내 제거 · 금지 문구 2건 + 하드코딩 문구 3언어화(모양 불변).
  - **2b**: 선택 확정 연출(화면 1B) · 라운드 전환 재설계 · 결승 3분할 · 첫 입장 팝업 — **클로드 디자인 합격 뒤 발행.**
- **이유**: D-20(정본 = 클로드 디자인)과 2026-08-31 확정 순서(클로드 디자인 → Claude Code). 네 화면의 디자인 실물이 없어 모양을 지어내지 않기 위해 분리했다. 원장 D-26.
- **대관 연출(킥 Phase F · ARENA-1a · M7의 ceremony 계측 2종 · R9~R11)은 2026-09-20 대표 결정으로 폐기** — 크라운 카드가 그 역할을 맡는다. 원장 D-24에 기록됨. 결승에서 고르면 지금처럼 바로 크라운 카드로 간다.

### §12-G · PR 2b 시작 기록 (2026-09-23)
- **디자인 정본**: `docs/design/claude-design/Arena_Match_Stage_v2_27boards_2026-09-22.dc.html` — 아트보드 **27장** (1~9 매치 무대(PR 1) · 10~14 선택 확정 연출 · 15~18 라운드 전환 · 19~23 결승 3분할 · 24~26 첫 입장 팝업 · 27 확정 정지 상태). 2026-09-22 대표 합격.
