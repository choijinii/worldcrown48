...
description: Use this skill to generate well-branded interfaces and assets for WorldCrown48 (월크48) — global fan-voting platform launching with the 2026 North American World Cup. Contains the v2.4 Twilight Stadium contract (v2.3 + AI-Report footer-only lock) (color · type · spacing · components), brand SVGs, fonts, and a clickable web UI kit. Strict ruleset — Round Scope Lock (Round labels appear only in the round-transition announcement between rounds, never on the Match VS Battle surface), no LIVE on TournamentCard, no Vote Rate on Match VS, "✦ AI-Report" only (news article footer only).
user-invocable: true
---
...
1. **`README.md`** — the v2.4 contract (extends v2.3 with AI-Report footer-only standard). Twelve inviolable rules, the seven-domain map, color · type · spacing foundations, the terminology contract.
...
* **Round labels appear only inside the round-transition announcement** — the full-screen interstitial that appears BETWEEN rounds, naming the round the Voter just completed and the next round. Never on the Match VS Battle surface, never on TournamentCard, Launch Pad, GNB, footers, or anywhere else. The Voter is a player, not a spectator — progress HUDs like `"N강 · X/Y"` are prohibited because they break the player experience.
