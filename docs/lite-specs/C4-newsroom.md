> 🛑 **2026-07-26 STALE — 구현 참조 금지.** 이 문서는 **ND-1 News Desk(`docs/lite-specs/ND1-news-desk.md`)로 대체된 스테일 스펙**이다.
> 미디어 피벗(2026-07-21) 이후 뉴스는 **외부 API 수집(GNews)이 아니라 자체 생산 승인제 뉴스 팩토리**로 확정됐다. 이 문서의 GNews·news_cache·키워드 뉴스 파이프라인을 **구현하지 말 것.** 후계 단일 진실 = ND-1 lite-spec. (이 배너는 정합성 참조용으로만 남긴다.)
>
