...
# WorldCrown48 (월크48) — 프로젝트 마스터 스킬 v1.6
...
| **생성 파일이 v4.8 설계서와 불일치** | **첨부 파일만 분석, 설계서 미확인** | **아래 10-7 재발 방지 규칙 참조** |

---

### 10-7. ⛔ 실제 발생한 오류 기록 — 설계서 미확인 파일 생성 (2026-05-14)

> 같은 실수가 반복되지 않도록 발생 경위, 결과, 재발 방지 규칙을 영구 기록합니다.

#### 발생 경위

대표님이 기존 `docs/lite-specs/` 폴더의 파일 12개를 첨부 파일로 업로드하여 업그레이드를 요청했을 때, Claude(48티오)가 **첨부 파일 12개만 분석하고 프로젝트 지식에 있는 v4.8 설계서를 먼저 확인하지 않은 채** 파일을 생성했습니다.

#### 결과 (대표님이 직접 수행해야 했던 작업)

```
- 파일 4개 통합 (설계서 기준 에이전트 1개 = 파일 1개인데 여러 파일로 쪼개져 있었음)
- 파일 3개 통합 (동일 이유)
- 파일 7개 개명 (에이전트 ID 기준이 아닌 번호 기준으로 잘못 생성됨)
→ 대표님 시간 낭비 + 불필요한 수작업 발생
```
...
```
✅ 올바른 우선순위:
   프로젝트 지식(v4.8 설계서) > 첨부 파일 > 대화 내용

❌ 실제로 한 것:
   첨부 파일만 분석 → 설계서와 다른 구조로 파일 생성
   (v4.8 설계서의 14개 에이전트 배치 맵을 확인하지 않음)
```

#### ⛔ 재발 방지 규칙 — 파일 작업 시 필수 절차 (절대 준수)

```
파일 생성·수정·개명·통합 요청이 들어오면 반드시 이 순서를 따를 것:

Step 1: project_knowledge_search → "에이전트 배치 맵 v4.8" 확인
         → 14개 에이전트 ID·도메인·모듈 구조 파악

Step 2: project_knowledge_search → 해당 작업 관련 설계서 섹션 확인
         → 요청된 파일/기능이 설계서 어디에 해당하는지 매핑

Step 3: 첨부된 파일과 설계서 내용 비교
         → 불일치 항목 목록화 후 대표님께 먼저 보고

Step 4: 설계서 기준으로 파일 구조 결정 후 작성

❌ 절대 금지: 첨부 파일만 보고 설계서 미확인 상태에서 파일 생성·수정
❌ 절대 금지: 설계서에 없는 파일 구조·네이밍으로 재편
❌ 절대 금지: "설계서 확인 없이도 충분히 알 것 같다"는 판단으로 진행
```

#### 올바른 파일 네이밍 원칙 (v4.8 에이전트 ID 기준)
...
올바른 예:
  A0-launch-pad.md        (에이전트 A-0, Domain 0)
  C1-vote-engine.md       (에이전트 C-1, Domain 3)
  G1-admin-dashboard.md   (에이전트 G-1, Domain 6)

틀린 예:
  07-the-pitch.md         (번호 기준 — 에이전트 ID와 무관)
  03-cookie-consent.md    (E-1 에이전트의 일부를 별도 파일로 분리)
  06-auth-vote-gate.md    (D-1 에이전트의 일부를 별도 파일로 분리)
```
...
| 버전 | 날짜 | 주요 내용 |
|------|------|-----------|
| **v1.6** | **2026-05-14** | **⛔ 오류 기록 + 재발 방지 규칙 추가** |
| | | ★ PART 10-7 신규: 설계서 미확인 파일 생성 오류 영구 기록 |
| | | ★ 재발 방지 4단계 절차 확립: 설계서 확인 → 매핑 → 비교 → 작성 |
| | | ★ 파일 네이밍 원칙 명문화: 에이전트 ID 기준 (번호 기준 금지) |
| | | ★ 10-6 문제 해결책 표에 "설계서 미확인" 항목 추가 |
| | | 배경: lite-specs 12개 파일을 설계서 미확인 상태로 업그레이드하여 |
| | | 대표님이 직접 통합 7건, 개명 7건 수작업을 해야 했던 사고 |
...
*© 2026 WorldCrown48 | 작성: 48티오 | ProjectSkill v1.6 | CONFIDENTIAL*
