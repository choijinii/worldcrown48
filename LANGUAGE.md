...
| **~~⚠️ 의도된 예외~~ 폐기** | ~~닉네임이 없는 사용자의 기본 표시 이름은 "Voter" 유지~~ → **Voter 예외 폐기 → 팬 / Fan (2026-09-24 대표 · D-32).** `UserDropdown`·`UserAvatar` 기본 표시 이름에 적용 |
...
**판(Run)·선택의 표시 용어 (2026-09-07 대표 확정 — v2.2):**
| 시스템 층 | 표시 층 ko | 표시 층 en / es | 이유 |
| --- | --- | --- | --- |
| **판 / Run** (§2 정의·코드·문서 불변) | **"참여" · "N번 참여" · "참여 횟수" · "오늘의 서비스(3번 참여)"** — '판'을 아주 금지하진 않되 주된 표현은 순화 | en "entry / entries"(신규 문구) · 기승인 문구의 "run"은 유지 / es "participación" · 기승인 "partida" 유지 | '판'은 한국어에서 놀음·도박·게임 연상이 강해 고급스럽지 않다(대표) |
...
| **선택 수** (화면 표기) | **Vote Count** | `voteCount` (코드 이름 불변) | 특정 Contestant이 받은 선택의 합계. **절대 수치는 차트 포함 어디에도 표시하지 않는다.** 화면·운영자 문구의 표기는 **"선택 수"**(2026-09-24). ★ **Vote Count(선택 수)와 Crown Score는 완전히 다른 개념** — §13 참조 |
| ~~득표율~~ **폐기** | ~~**Vote Rate**~~ | `voteRate` (코드 이름만 존속) | **폐기(2026-09-23 대표).** 화면에서 "Vote Rate / 득표율"을 쓰지 않는다. 차트에 나가는 수치는 **Crown Score** 하나뿐이다(§13). 정본의 **점유율**은 Vote Rate와 **분모가 다른 별개 값**이다(대결에서 뽑힌 횟수 ÷ 대결에 나온 횟수) — 동의 표기가 아니다. 코드의 `rate` 필드는 이상 징후(T-1·T-2) 판정용으로만 남는다 |
...
| **일일 판 한도** | **Daily Run Limit** | dailyRunLimit | Voter 1명이 **하나의 Tournament에서 하루(KST 기준)에 참가할 수 있는 판 수 = 5판**. 대회마다 각각 5판(대회 A 5판 · 대회 B 5판). **판마다 대진표가 새로 섞이고 Crown Card가 1장씩 생성**되며, 5판 모두 랭킹에 반영된다. 비로그인(게스트)의 한도는 별도 용어 **게스트 일일 판 한도**(아래 행) — ★ v2.1(2026-09-06)에서 1판 → 3판으로 개정. ★ v2.0 (2026-09-03 대표 확정) — 폐기된 "Daily Participation Limit(하루 새 대회 5개, HF-1)"을 대체. 정본 문서: `outputs/참가규칙_정본v2.1_판Run_2026-09-07.html` |
| **게스트 일일 판 한도** | **Guest Daily Run Limit** | guestDailyRunLimit (`GUEST_DAILY_RUN_LIMIT`) | 비로그인(익명 uid) Voter가 **하루(KST) 통틀어 돌 수 있는 판 수 = 3판.** Tournament를 가로질러 세며(대회 자유 선택), **Tournament 수가 늘어도 3판 고정**(MVP1.5 유저 생성 Tournament가 열려도 늘지 않는다). 4판째부터 로그인. 게스트 판도 Crown Card가 생성되고 **공유는 열려 있으나 저장·다운로드는 로그인 필요.** **게스트의 선택은 랭킹 집계에서 제외**된다(2026-09-07 확정: PR 2 배포 이후 기록부터, 소급 없음). 숫자 3은 조정 가능한 값 — 코드 상수 한 곳(`lib/run/guestRun.ts`)에서만 정의. ★ v2.1 (2026-09-06 대표 확정) — v2.0의 "하루 통틀어 1판"을 대체. 근거: `marketing/00_strategy/게스트정책_v2.1_결정.md` |
...
| 참가자, 후보자 | **Contestant** (한국어 표기 = **엔트리**) | 공식 용어는 Contestant — §15 참조 (D-31) |
| Candidate | **Contestant** | v1.2에서 Contestant로 통일 |
| 득점 / 표 / 득표율 / Vote Rate | **선택 · 선택 수(Vote Count)** | "표"는 낱말 자체가 금지(2026-09-07). "득표율 / Vote Rate"는 폐기(2026-09-23) — 차트의 수치는 **Crown Score** |
| 랭킹 (화면 이름으로) | **차트 (Chart)** | 대회별 순위표의 공식 이름은 차트 (D-31 계열 · 정본 §1). "랭킹"은 **차트 속 등수**를 뜻하는 일반 명사로만 남는다 ("랭킹 2위") |
...
| "표" (낱말 자체 — 게스트 표 · 내 표 · 표를 넣다 · 부정표) | **선택** (게스트의 선택 · 내 선택 · 선택을 넣다) | ★ v2.2 (2026-09-07 대표 확정): 단위 금지에서 **낱말 금지로 승격.** 제품이 "투표"에서 "선택"의 개념으로 넘어갔으므로 '표'가 남으면 과거의 투표 의미(집계·머릿수)로 읽힌다. 문서·화면·에이전트 대화 모두 적용. `votes`·`Vote`·`onVote`는 DB·코드 내부 이름으로만 존치 |
| 비로그인 하루 1판 / 게스트 1판 / 무료 1판 | **게스트 일일 판 한도 (하루 통틀어 3판)** | ★ v2.1 (2026-09-06 대표 확정): 1판 정의는 폐기. 09-05 승인 문구 "오늘의 무료 1판"도 무효 — 재승인 문구로 교체 |
...
> **★ v2.1 개정(2026-09-06 대표 확정) — 위 "1회 완주"는 옛 수치다.** Guest Run이라는 용어(비로그인 Voter의 한 판 완주)는 그대로이되, **허용 판 수는 §2 "게스트 일일 판 한도" = 하루 통틀어 3판**이 정본이다. `guestTournamentId`(같은 Tournament 이어하기 판정용)는 3판·복수 Tournament 구조에서 폐기되고 이어하기 판정은 회차(`decideRun`의 continue)가 맡는다. 게스트 공유 개방·저장 잠금·랭킹 제외도 §2 행 참조.
...
### Crown Score  ★ v1.0 (2026-09-23 대표 확정)

> **정본: `marketing/00_strategy/CROWN_SCORE_v1.0.md`.** 이 문서와 정본이 다르면 정본이 이긴다.

| 항목 | 내용 |
|---|---|
| **English Term** | `Crown Score` |
| **한국어** | (번역 불가 — 영문 원형 사용) |
| **정의** | 차트의 순위를 정하는 점수. **Crown Score = (순위점수율 × 0.4 + 우승율 × 0.3 + 점유율 × 0.3) × 1000** → **0~1000 정수** |
| **순위점수율** | (받은 순위 점수 합계 ÷ 대회 전체 판수) ÷ 10. 판 1회의 순위 점수 = **1등 10 · 2등 5(2명) · 3등 2(3명)**, 합계 26 고정 |
| **우승율** | 1등 횟수 ÷ 대회 전체 판수 |
| **점유율** | **대결에서 뽑힌 횟수 ÷ 대결에 나온 횟수.** 결승(3인 중 1명 선택)도 대결 1번으로 센다 |
| **세는 범위** | 한 대회 안에서, **완주한 판만.** 게스트 판은 제외(D-02) |
| **차트 표시** | **Crown Score 정수 하나만** 표시(2026-09-24 대표). 세 비율은 캐시에 저장만 한다. 점수 옆 **? 설명창 = 계산식을 쉬운 말로 알려 주는 공통 문구 하나**(Contestant별 수치가 아님) |
| **최소 판수** | 대회 전체 완주 판수 **10판** 미만이면 점수를 보여 주지 않고 기다림 안내. **차트로 가는 길은 숨기지 않는다** |
| **⚠️ Vote Count와의 구분** | **점수(Crown Score) ≠ 선택 수(Vote Count).** 절대 수치는 트래픽 종속이라 차트 포함 어디에도 표시하지 않는다 |
| **~~폐기~~ (2026-09-23)** | ~~Crown Score = 우승비율 × 50% + 점유율 × 50%~~ (2026-07-11 산식) · ~~점유율 = 받은 표 ÷ 대회 전체 표~~ · ~~승률(Win Rate) 표시~~ · ~~우승비율 → 점유율 → 승률 3종 나열~~ — 전부 위 v1.0으로 대체 |
...
| **정의** | Crown Score와 그 재료(순위점수율·우승율·점유율)의 사용 범위 잠금: **차트 도메인 + AI 뉴스 기사 소재만 허용.** 매치(VS Battle)·Crown Card 사용 절대 금지 |
| **관계** | 기존 "Vote Rate는 랭킹 화면에서만" 원칙의 확장판. 지표 이름은 Crown Score v1.0(2026-09-23)으로 갱신됐다 |
...
| **버전** | **날짜** | **주요 변경** |
| --- | --- | --- |
| **v2.2** | **2026-09-07** | **★ 참가 규칙 v2.1 전파(게스트 정책, 2026-09-06 대표 확정) + '표' 낱말 금지 승격 + §1 표시 용어 층에 판→"참여" 순화·"선택"·히어로 "예측·배당" 삭제 행 추가(2026-09-07 대표 확정).** §2 신규 행 **게스트 일일 판 한도 = 하루 통틀어 3판**(대회 수 무관 고정 · 공유 개방 · 저장 잠금 · 게스트의 선택 랭킹 제외, 소급 없음) · §7 금지어 2행 추가("표" 낱말 자체 → "선택" · "무료 1판" 폐기) · §12 Guest Run에 v2.1 각주. 일일 판 한도 행의 게스트 문장은 새 행을 가리키도록만 수정(로그인 규칙 무변경) |
| **v2.1.1** | **2026-09-03** | (소급 기록) **★ 참가 규칙 v2.0 = 판(Run) 전파.** §2 신규 행 판·회차·일일 판 한도(대회당 하루 5판) · §7 금지어 3행(Daily Vote Limit·Daily Participation Limit·"표" 단위). 당시 변경 이력 표에 기록되지 않아 09-07에 소급 기재 |
...
## 15. 차트 · 엔트리 (2026-09-23 ~ 09-24 대표 확정)

> 근거: 정본 `marketing/00_strategy/CROWN_SCORE_v1.0.md` · 결정원장 **D-23 · D-30 · D-31 · D-32**

### 차트 (Chart)

| 항목 | 내용 |
|---|---|
| **English Term** | `Chart` / `Charts` |
| **한국어** | **차트** |
| **정의** | **대회별 결과 순위표.** 화면 메뉴·제목, 뉴스룸 카테고리, 기사에서 쓰는 공식 이름 |
| **순위 기준** | Crown Score 높은 순 (§13) |
| **공개 범위** | **마감 전에도, 로그인하지 않아도 상시 공개** (D-30). 마감 전 잠금(W-7)은 폐기 |
| **왜 이름을 바꿨나** | "랭킹"은 뜻은 맞지만 흔한 말이라 인용되는 이름표가 되지 못한다. 빌보드 차트처럼 "월크48 차트"는 출처로 기억된다 |
| **내부 이름** | 주소 `/arena/[id]/ranking` · 컬렉션 `ranking_cache` · 변수 `rate` 등은 **바꾸지 않는다** (정본: "내부 이름은 개발 편한 대로") |

### 랭킹 (일반 명사)

| 항목 | 내용 |
|---|---|
| **정의** | **차트 안에서의 등수.** 고유명사가 아니라 일반 명사다 |
| **쓰는 곳** | 기사 본문 — "○○○ 차트에서 **랭킹 2위**" |
| **쓰지 않는 곳** | 화면 메뉴·제목·뉴스룸 카테고리 이름 (그 자리는 **차트**) |

### 엔트리 (Contestant의 한국어 표기)  ★ D-31

| 항목 | 내용 |
|---|---|
| **English / es / 코드** | **Contestant** (불변) |
| **한국어 표기** | **엔트리** |
| **정의** | 한 대회에 출전하는 한 개의 대상 (사람·그룹·캐릭터·음식 등). 예: "이번 대회 엔트리 48개" |
| **허용 변형** | 기사·설명 문장에서 전체를 가리킬 때 **"출전명단"**, 줄여서 **"명단"** |
| **왜** | "Contestant"는 한국 팬에게 낯선 영어다(Champion·Match·Tournament와 달리 외래어로 자리 잡지 않았다). "엔트리"는 스포츠에서 흔한 말이라 세계관(D-07)과 맞고, 사람이 아닌 대상에도 어색하지 않다 |
| **⚠️ 화면 적용 시점** | **The Pitch 개편 때** (ARENA-1 PR 3은 **사전 등재만**). 현재 화면 문구는 아직 바뀌지 않았다 |
| **금지** | **"참가자"** — §7 금지 용어 |

### Pick Now / 참가하기 / Elige ahora  ★ D-23

| 항목 | 내용 |
|---|---|
| **정의** | 팬을 아레나로 보내는 행동 버튼(CTA)의 문구. ko **참가하기** · en **Pick Now** · es **Elige ahora** |
| **대체한 것** | ~~"Vote Now"~~ (2026-09-23 폐기) — 투표 개념을 화면에 남기던 마지막 큰 글자 |
| **⚠️ 계측** | 이벤트 이름 `a1_gnb_cta_vote_now` 는 **코드 식별자라 바꾸지 않는다** (지표 연속성) |

### Choe-ae (최애)  ★ D-25

| 항목 | 내용 |
|---|---|
| **정의** | 결승 화면 제목 **한 곳에만** 쓰는 낱말. 로마자 표기 **Choe-ae** 고정 |
| **범위** | 그 한 자리 밖에서는 쓰지 않는다 |

---

*© 2026 WorldCrown48 | 작성: 48티오 | LANGUAGE.md v2.2 (2026-09-24) | CONFIDENTIAL*
